...
static void lv_100ask_2048_constructor(const lv_obj_class_t * class_p, lv_obj_t * obj)
{
    ...
    for (index = 0; index < game_2048->map_count; index++) {
        ...
        if (((index + 1) % 5) == 0)
        {
            game_2048->btnm_map[index] = lv_malloc_zeroed(2);
        ...
        {
            game_2048->btnm_map[index] = lv_malloc_zeroed(5);
            strcpy(game_2048->btnm_map[index], " ");
        }
    }
...
static void btnm_event_cb(lv_event_t * e)
{
    ...
    if (code == LV_EVENT_DRAW_TASK_ADDED)
    {
        ...
        //LV_LOG_WARN("id %d, type %d, obj %d, part %d, size %d\n", id, draw_task->type, base_dsc->obj, base_dsc->part, base_dsc->dsc_size);
        lv_draw_fill_dsc_t *fill_draw_dsc = lv_draw_task_get_fill_dsc(draw_task);
        lv_draw_label_dsc_t *label_draw_dsc = lv_draw_task_get_label_dsc(draw_task);

        /*Change the draw descriptor the button*/
        if ((id >= 0) && (id < 16) && base_dsc->part)
        {
            uint16_t x, y, num;
        ...
        else if ((id == 0) && !base_dsc->part)
        {
            lv_draw_fill_dsc_t *fill_draw_dsc = lv_draw_task_get_fill_dsc(draw_task);
            if (fill_draw_dsc)
            {
                fill_draw_dsc->radius = 5;
                fill_draw_dsc->color = LV_100ASK_2048_BG_COLOR;
            }
        }
